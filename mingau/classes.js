  ...
  constructor(x, y, l, a, name, id, txt) {
    ...
    this.img = {
      pos: createVector(
        x -
          10 -
          (((imagem.talk[id].width * height) / 5) * 4) /
            imagem.talk[id].height /
            2,
        y - 10 - ((height / 5) * 4) / 2
      ),
      size: createVector(
    ...
    if (this.display.size.x > 10) {
      ...
      if (this.display.size.x > this.display.max.x - 20) {
        ...
        text(
          ...
          this.img.size.x + 50,
          this.img.size.y,
          (width / 5) * 4 - 10 - (this.img.size.x + 50)
        );
      }
    }
    if (this.seechar) {
...
class GUI {
  ...
  show() {
    if (this.type != "menu") {
      if (
        this.see && mouseOverR(this.icon.pos, this.icon.size.x, this.icon.size.y, 1) ||
        (this.see &&
          !mouseOverR(
            this.display.pos,
            this.display.size.x,
    ...
    if (this.type != "menu") {
      if (this.see) {
        ...
        strokeWeight(1);
        imageMode(CORNER);
        if(this.name == "Pagina 1" || this.name == "Pagina 2" || this.name == "Pagina 3" ){
          image(imagem.origame[this.id],this.display.pos.x+10,
            this.display.pos.y+70,this.display.size.x-20,
            (imagem.origame[this.id].height * (this.display.size.x-20)) / imagem.origame[this.id].width);
        } else {
          image(imagem.obj[this.id],this.display.pos.x+10,
            this.display.pos.y+70,this.display.size.x-20,
            (imagem.obj[this.id].height * (this.display.size.x-20)) / imagem.obj[this.id].width);
        }
